...
for frame in (login, register, mainMenu, passwordStructure, social_media, confirm,
			  randomised_password, deny, code, codeoption, codePassword, display,barcode_display, notes, notesbarcode,
...
def Proceed_QRCODE():
	print(f'QRorBAR_Option2: {QRorBAR_Option2}')
	qrcode_notes = QR_textbox.get("1.0","end"+"-1c")
	QRcode_PNG = QRPNG_name.get()
	if M3_OoiKherNing_1191100876_MODIFIED.duplicatePNG_checker(QRcode_PNG) == True:
		...
		raise_frame(display)
		canvas_qrcode = Canvas(display, width = 400, height = 400)
		canvas_qrcode.grid(row= 3, column=0, columnspan=2, padx=20, pady=20)
		img_var = Image.open('./QRcode/'+QRcode_PNG+'.png')
		img = ImageTk.PhotoImage(img_var)
		canvas_qrcode.image = img
		canvas_qrcode.create_image(20, 20, anchor=NW, image=img)
...
def Proceed_BARCODE():
	...
	if M3_OoiKherNing_1191100876_MODIFIED.duplicatePNG_checker(BARcode_PNG) == True:
		...
		if selectedcategory_QRorBAR_code == 'customise':
			cust_cat = custom_cat.get()
			cust_subject = custom_subject.get() 
			M3_OoiKherNing_1191100876_MODIFIED.barcode(cust_cat,cust_subject,barcode_notes,BARcode_PNG)
		else:
			M3_OoiKherNing_1191100876_MODIFIED.barcode(selectedcategory_QRorBAR_code,QRorBAR_Option2,barcode_notes,BARcode_PNG)
		
		raise_frame(barcode_display)
		canvas_barcode = Canvas(barcode_display, width = 1700, height = 300)
		canvas_barcode.grid(row= 3, column=0, columnspan=2, padx=20, pady=20)
		img_var = Image.open('./BARcode/'+BARcode_PNG+'.png')
		img = ImageTk.PhotoImage(img_var)
...
entRandomisedPassword = Entry(randomised_password, width=55, state="readonly" , textvariable=password_displayed)
...
btnBack = Button(code, text="BACK", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:backButton())
...
lblCust.grid(row=1,column=0, columnspan=2, padx=0, pady=20, sticky="news")
...
btnNext = Button(customise, text= "NEXT", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:raise_frame(code))
btnNext.grid(row=5, column=0, padx= 250, pady=30, sticky="news")

btnBack = Button(customise, text= "BACK", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:raise_frame(codeoption))
btnBack.grid(row=5, column=1, padx= 150, pady=30, sticky="news")
...
QR_textbox.grid(row=2, columnspan=2, padx=200, pady=0)
...
entPNG_name = Entry(notes, width=20, textvariable = QRPNG_name, bg="light grey")
...
btnBack = Button(notes, text= "BACK", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:raise_frame(codeoption))
btnBack.grid(row=10, column=2, padx= 30, pady=10, sticky="E")
...
btnCode.grid(row=5, column=0, columnspan=2, padx=250, pady=20)
...
lbldata = Label(notesbarcode, text="*Recomended for minor data that have only at most 10 letters*", font="fixedsys 6  bold underline", bg="#222930", fg="#E9E9E9")
lbldata.grid(row=2, column=0, columnspan=2, padx=320, pady=20, sticky="news")
...
BAR_note = StringVar()
BAR_textbox = Entry(notesbarcode,width = 20,textvariable = BAR_note, bg="light grey")
BAR_textbox.grid(row=3, columnspan=2, padx=200, pady=0)
...
entPNG_name = Entry(notesbarcode, width=20, textvariable = BARPNG_name, bg="light grey")
entPNG_name.grid(row=9, column=1, sticky=W)

btnNotes = Button(notesbarcode, text="PROCEED",font="courier 10", bg="#4EB1BA", fg="#E9E9E9", command=lambda:[confirmData,Proceed_BARCODE()])
btnNotes.grid(row= 10, column=1, padx=30, pady=10, sticky=W)

btnBack = Button(notesbarcode, text= "BACK", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:raise_frame(codeoption))
btnBack.grid(row=10, column=2, padx= 30, pady=10, sticky="E")
