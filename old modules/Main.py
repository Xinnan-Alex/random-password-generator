...
def Proceed_QRCODE():
    ...
    if M3_OoiKherNing_1191100876_MODIFIED.duplicatePNG_checker(QRcode_PNG) == True:
        ...
        raise_frame(display)
        canvas = Canvas(display, width = 400, height = 400)
...
def Proceed_BARCODE():
    barcode_notes = BAR_textbox.get("1.0","end"+"-1c")
    ...
    if M3_OoiKherNing_1191100876_MODIFIED.duplicatePNG_checker(BARcode_PNG) == True:
        ...
        raise_frame(display)
        canvas = Canvas(display, width = 900, height = 220)
        canvas.grid(row= 3, column=0, columnspan=2, padx=20, pady=20)
...
btnBack = Button(socialmedia, text= "BACK", font="courier 10 bold", fg="#E9E9E9", bg="#4EB1BA", command=lambda:raise_frame(mainMenu))
...
def category_onselect(option):
    ...
    if index == 4:
        ...
        confirmCustomise()
    print(selectedcategory_QRorBAR_code)
...
BAR_textbox = Text(notesbarcode, height=20, width=40)
BAR_textbox.grid(row=2, columnspan=2, padx=200, pady=0)
