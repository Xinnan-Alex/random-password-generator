...
filemenu.add_command(label="Home", command=lambda:raise_frame(mainMenu))
filemenu.add_separator()

filemenu.add_command(label="Generate Password", command=lambda:raise_frame(passwordStructure))
filemenu.add_command(label="Password Manager", command=lambda:raise_frame(password_manager))
filemenu.add_command(label="QR code / Barcode", command=lambda:raise_frame(code))
filemenu.add_command(label="Log out" , command=lambda:raise_frame(login))
filemenu.add_separator()
...
def account_verify():
    status = account_management.datachecker(login_username,login_password)
    if status == True:
        global currentuser
